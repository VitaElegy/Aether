use crate::domain::{
    models::{Article, ContentBody, ContentStatus, Node, NodeType, PermissionMode, UserId},
    ports::{ArticleRepository, PermissionRepository, RepositoryError, UserRepository},
};
use axum::{
    ...
    match node.permission_mode {
        ...
        PermissionMode::Private => {
            if let Some(u) = user {
                ...
                PermissionRepository::has_relation(repo, node.id, "node", "editor", u.id, "user")
                    .await
                    .unwrap_or(false)
            } else {
                false
...
async fn check_edit_permission(
    ...
    PermissionRepository::has_relation(repo, node.id, "node", "editor", user.id, "user")
        .await
        .unwrap_or(false)
}
...
pub async fn create_content_handler(
    ...
    if let Ok(Some(_)) = state.repo.find_by_title(&payload.title).await {
        return (
            StatusCode::CONFLICT,
            Json(serde_json::json!({ "error": "Article with this title already exists" })),
        )
            .into_response();
    }
    ...
    if content_type == "Folder" || content_type == "Directory" {
        ...
        let slug = payload.slug.unwrap_or_else(|| {
            format!(
                "{}-{}",
                payload.title.to_lowercase().replace(" ", "-"),
                &id.to_string()[..8]
            )
        });
        ...
        let derived_data_value = {
            ...
            Some(
                serde_json::to_value(crate::domain::sentence_parser::SentenceParser::parse(
                    &text_to_parse,
                    None,
                ))
        ...
        match ArticleRepository::save(&*state.repo, article, UserId(user.id), payload._reason).await
        {
            ...
            Err(RepositoryError::DuplicateTitle(msg)) => (
                StatusCode::CONFLICT,
    ...
    match existing_item {
        crate::domain::models::ContentItem::Node(mut n) => {
            if payload.content_type.as_deref() == Some("Article") {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({ "error": "Cannot change type to Article" })),
                )
                    .into_response();
            }
        ...
        crate::domain::models::ContentItem::Article(existing) => {
            ...
            let derived_data_value = {
                // Extract old map
                let old_map = existing.derived_data.and_then(|v| {
                    serde_json::from_value::<crate::domain::sentence_parser::SentenceMap>(v).ok()
                });
            ...
            let updated_article = Article {
                node: Node {
                    ...
                    knowledge_base_id: payload
                        .knowledge_base_id
                        .or(existing.node.knowledge_base_id),
                    r#type: NodeType::Article,
...
pub async fn list_content_handler(
    ...
    if let Some(uid) = user.0.as_ref() {
        ...
        if is_self_view {
            ...
            // Collect Article IDs
            let article_ids: Vec<Uuid> = items
                .iter()
                .filter_map(|i| {
            ...
            if !article_ids.is_empty() {
                ...
                match state.repo.find_drafts_by_article_ids(article_ids).await {
                    Ok(drafts) => {
                        ...
                        let draft_map: std::collections::HashMap<
                            Uuid,
                            (String, serde_json::Value, chrono::DateTime<chrono::Utc>),
                        > = drafts
                            .into_iter()
                        ...
                        for item in items.iter_mut() {
                            if let crate::domain::models::ContentItem::Article(ref mut a) = item {
                                if let Some((d_title, d_body, d_updated)) =
                                    draft_map.get(&a.node.id)
                                {
                                    ...
                                    a.body = crate::domain::models::ContentBody::Markdown(
                                        draft_body_str,
                                    );
                                }
                            }
    ...
    match ArticleRepository::find_by_id(&*state.repo, &id).await {
        Ok(Some(item)) => {
            ...
            if check_view_permission(&state.repo, node, &user.0).await {
                ...
                let collaborator_ids = PermissionRepository::get_collaborators(
                    &*state.repo,
                    node.id,
                    "node",
                    "editor",
                ...
                for uid in collaborator_ids {
                    if let Ok(Some(u)) =
                        UserRepository::find_by_id(&*state.repo, &UserId(uid)).await
                    {
...
pub async fn list_collaborators_handler(
    ...
    if item.node().author_id != user.id {
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({ "error": "Only owner can list collaborators" })),
        )
            .into_response();
    }
    ...
    let collaborator_ids =
        match PermissionRepository::get_collaborators(&*state.repo, id, "node", "editor").await {
            Ok(ids) => ids,
            Err(e) => {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
    ...
    for uid in collaborator_ids {
        if let Ok(Some(u)) =
            UserRepository::find_by_id(&*state.repo, &crate::domain::models::UserId(uid)).await
        {
...
pub async fn add_collaborator_handler(
    ...
    if item.node().author_id != user.id {
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({ "error": "Only owner can add collaborators" })),
        )
            .into_response();
    }
    ...
    use crate::domain::ports::PermissionRepository;
    match PermissionRepository::add_relation(
        &*state.repo,
        id,
        "node",
        "editor",
...
pub async fn remove_collaborator_handler(
    ...
    if item.node().author_id != user.id {
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({ "error": "Only owner can remove collaborators" })),
        )
            .into_response();
    }
    ...
    // 2. Remove Relation
    use crate::domain::ports::PermissionRepository;
    match PermissionRepository::remove_relation(
        &*state.repo,
        id,
        "node",
        "editor",
...
pub async fn save_draft_handler(
    ...
    match state.repo.save_draft(id, payload.title, payload.body).await {
        Ok(_) => (
            StatusCode::OK,
            Json(serde_json::json!({ "status": "draft_saved" })),
        )
            .into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn publish_draft_handler(
    ...
    let (draft_title, draft_body) = match state.repo.find_draft_by_id(&id).await {
        ...
        Ok(None) => {
            return (
                StatusCode::NOT_FOUND,
                Json(serde_json::json!({ "error": "No draft found to publish" })),
            )
                .into_response()
        }
        Err(e) => {
    ...
    if let crate::domain::models::ContentItem::Article(existing_article) = article_item {
        ...
        match ArticleRepository::save(
            ...
            updated_article,
            UserId(user.id),
            Some("Published from Draft".to_string()),
        )
        .await
        {
            Ok(_) => (
        ...
        (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": "Target is not an article" })),
        )
            .into_response()
    }
}
...
pub fn router() -> axum::Router<crate::interface::state::AppState> {
    ...
    axum::Router::new()
        .route(
            "/api/content",
            post(create_content_handler).get(list_content_handler),
        )
        .route(
            "/api/content/:id",
        ...
        .route(
            "/api/content/:id/history/:version",
            get(get_content_version_handler),
        )
        .route(
            "/api/content/:id/diff/:v1/:v2",
            get(get_content_diff_handler),
        )
        .route(
            "/api/content/:id/collaborators",
            get(list_collaborators_handler).post(add_collaborator_handler),
        )
        .route(
            "/api/content/:id/collaborators/:uid",
