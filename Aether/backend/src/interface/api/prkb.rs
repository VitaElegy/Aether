...
use crate::domain::prkb::models::{Feed, Paper};
...
pub async fn get_inbox(
    ...
    match state.repo.get_inbox(q.limit.unwrap_or(50), q.offset.unwrap_or(0), q.unread_only.unwrap_or(false)).await {
...
pub async fn fetch_feeds(
    ...
    for feed in feeds_to_fetch {
        ...
        let items_result = if feed.feed_type == "arxiv" {
            state.arxiv_service.fetch_recent_by_category(&feed.url, 20).await
...
pub async fn list_papers(
    ...
    match state.repo.list_papers(50, 0).await {
