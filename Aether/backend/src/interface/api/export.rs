...
use crate::domain::ports::ExportFormat;
...
pub async fn export_node_handler(
    ...
    match state
        .export_service
        .export_node_with_comments(&id, format.clone(), Some(UserId(user.id)))
        .await
    {
        Ok(bytes) => {
...
pub fn router() -> axum::Router<crate::interface::state::AppState> {
    ...
    axum::Router::new().route("/api/export/:id", get(export_node_handler))
}
