...
pub async fn check_permission_handler(
    ...
    match service
        .check_permission(params.user_id, params.entity_id, &params.action)
        .await
    {
        Ok(allowed) => (
            StatusCode::OK,
...
pub async fn list_user_permissions_handler(
    ...
    match state
        .permission_service
        .get_user_explicit_permissions(user_id)
        .await
    {
...
pub async fn grant_permission_handler(
    ...
    match state
        .permission_service
        .grant_permission(payload.user_id, payload.entity_id, &payload.relation)
        .await
    {
        Ok(_) => (
            StatusCode::OK,
            Json(serde_json::json!({ "message": "Permission granted" })),
        )
            .into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn revoke_permission_handler(
    ...
    match state
        .permission_service
        .revoke_permission(payload.user_id, payload.entity_id, &payload.relation)
        .await
    {
        Ok(_) => (
            StatusCode::OK,
            Json(serde_json::json!({ "message": "Permission revoked" })),
        )
            .into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn break_glass_handler(
    ...
    match state
        .permission_service
        .break_glass_access(auth_user.id, payload.entity_id, &payload.relation)
        .await
    {
        Ok(_) => (
            StatusCode::OK,
            Json(
                serde_json::json!({ "message": "Break-Glass access granted. Audit log recorded." }),
            ),
        )
            .into_response(),
        Err(e) => {
...
pub fn router() -> axum::Router<AppState> {
    ...
    axum::Router::new()
        ...
        .route(
            ...
            get(list_user_permissions_handler),
        )
