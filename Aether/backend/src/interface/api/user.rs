...
pub async fn search_users_handler(
    ...
    tracing::info!(
        "Searching users: q='{}', limit={}, offset={}",
        query,
        limit,
        offset
    );
    ...
    match state.repo.search_users(&query, limit, offset).await {
        Ok(users) => {
            let dtos: Vec<UserSummary> = users
                .into_iter()
                .map(|u| UserSummary {
                    id: u.id.0,
                    username: u.username,
...
pub async fn delete_user_handler(
    ...
    match state
        .permission_service
        .check_permission(auth_user.id, SYSTEM_ROOT_ID, "manage_users")
        .await
    {
        Ok(allowed) => {
            if !allowed {
                ...
                return (
                    ...
                    Json(serde_json::json!({ "error": "Insufficient system permissions" })),
                )
                    .into_response();
            }
        }
    ...
    match state
        .repo
        .delete(&crate::domain::models::UserId(target_user_id))
        .await
    {
        Ok(_) => (
            StatusCode::OK,
