use crate::domain::{
    models::{permissions, ExperienceItem, User},
...
impl FromRequestParts<crate::interface::state::AppState> for AuthenticatedUser {
    ...
    async fn from_request_parts(
        parts: &mut Parts,
        state: &crate::interface::state::AppState,
        ...
        let auth_header = parts
            ...
            .map_err(|_| {
                (
                    StatusCode::UNAUTHORIZED,
                    Json(serde_json::json!({ "error": "Invalid token header" })),
                )
            })?;
        ...
        match auth_service.verify_token(token) {
            Ok(claims) => {
                ...
                match user_repo
                    .find_by_id(&crate::domain::models::UserId(id))
                    .await
                {
...
pub async fn login_handler(
    ...
    match state
        ...
        .await
    {
        Ok(claims) => match state.auth_service.generate_token(&claims) {
            Ok(token) => {
...
pub async fn update_user_handler(
    ...
    if auth_user.id != id {
        return (
            ...
            Json(serde_json::json!({ "error": "Cannot update other users" })),
        );
    }
