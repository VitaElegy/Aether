...
use crate::domain::models::{ContentBody, ContentItem, UserId};
use crate::domain::portability::models::{
    ExportSection, ExportSummary, ImportSummary, ProgressEvent,
};
...
impl PortabilityProvider for EnglishPortabilityProvider {
    ...
    async fn export(
        ...
        let manifest = EnglishPortabilityManifest {
            format: "aether-portability-export",
            provider_id: "english_v1",
            knowledge_base_id: kb_id,
            exported_at: Utc::now().to_rfc3339(),
            restorable: true,
            snapshot_path: Some("snapshot.akb"),
        };
        ...
        wtr.write_record(&["word", "definition", "status", "tags", "created_at"])
            .map_err(|e| e.to_string())?;
