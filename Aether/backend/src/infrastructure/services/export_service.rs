...
use crate::domain::ports::{
    ArticleRepository, CommentRepository, ExportFormat, ExportService, MemoRepository,
    RepositoryError,
};
...
impl ExportService for DataExportService {
    async fn export_node_with_comments(
        ...
        if let Some(article) = article_opt {
            let comments = self
                .comment_repo
                .get_comments(node_id)
                .await
                .unwrap_or_default();
            let comment_texts: Vec<String> = comments
                .into_iter()
                .map(|c| format!("{}: {}", c.user_name.unwrap_or("Anon".into()), c.text))
                .collect();
            ...
            match format {
                ...
                .unwrap()),
                ExportFormat::Html => Ok(format!(
                    "<html><h1>{}</h1><p>{}</p></html>",
                    article.node.title, content_str
                )
                .into_bytes()),
            }
