...
use crate::domain::prkb::models::{Author, Feed, InboxItem, Paper, Signals, Venue};
...
use crate::infrastructure::persistence::entities::{
    prkb_authors, prkb_feeds, prkb_inbox, prkb_papers, prkb_papers_authors, prkb_signals,
    prkb_venues,
};
...
impl PrkbRepository for PostgresRepository {
    ...
    async fn get_inbox(
        ...
        let mut query = prkb_inbox::Entity::find().order_by_desc(prkb_inbox::Column::PublishDate);
        ...
        if let Some(venue) = paper.venue {
            ...
            if let Err(e) = res {
                if !e.to_string().contains("duplicate key") {
                    // Should be handled by on_conflict
    ...
    async fn list_papers(
        ...
        // Step 1: Fetch Papers
        let mut query = prkb_papers::Entity::find().order_by_desc(prkb_papers::Column::SavedAt);
        ...
        let signals_map: std::collections::HashMap<Uuid, Signals> = signals_models
            .into_iter()
            .map(|s| {
                (
                    s.paper_id,
        ...
        let result = paper_models
            ...
            .map(|p| {
                ...
                Paper {
                    ...
                    venue,
                    signals,
                    metadata: serde_json::from_value(p.metadata.unwrap_or(serde_json::json!(null)))
                        .ok(),
                }
            })
            .collect();
