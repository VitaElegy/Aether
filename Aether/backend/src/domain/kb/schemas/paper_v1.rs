...
impl BlockSchema for PaperSchema {
    fn validate(&self, payload: &Value) -> Result<(), SchemaError> {
        ...
        if payload.get("title").and_then(|v| v.as_str()).is_none() {
            return Err(SchemaError::ValidationFailed(
                "Missing or invalid 'title' field in paper block".into(),
            ));
        }
        ...
        match ingest_status {
            ...
            Some(other) => {
                return Err(SchemaError::ValidationFailed(format!(
                    "Invalid ingest_status: {}",
                    other
                )))
            }
            None => {
                return Err(SchemaError::ValidationFailed(
                    "Missing 'ingest_status' field in paper block".into(),
                ))
            }
        }
    ...
    fn to_searchable_text(&self, payload: &Value) -> String {
        ...
        format!("{} {} {} {}", title, authors, venue, abstract_text)
            .trim()
            .to_string()
    }
}
