...
mod tests {
    ...
    use crate::domain::kb::registry::{BlockSchema, SchemaRegistry};
    use anyhow::Result;
    use serde_json::json;
    ...
    impl BlockSchema for MockEnglishBlock {
        fn validate(
            &self,
            payload: &Value,
        ) -> Result<(), crate::domain::kb::registry::SchemaError> {
            if payload.get("text").is_none() {
                return Err(crate::domain::kb::registry::SchemaError::ValidationFailed(
                    "Missing 'text' field".into(),
                ));
            }
            Ok(())
        }
