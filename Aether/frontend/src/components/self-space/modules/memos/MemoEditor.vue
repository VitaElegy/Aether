    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        ...
        >
            ...
            <div class="px-6 pt-5 pb-2 flex items-center justify-between shrink-0 z-10 relative">
                ...
                <div class="flex items-center gap-1.5">
                    <button
                        ...
                        :class="[colorBg(c), localData.color === c ? 'ring-2 ring-offset-2 ring-black/20 dark:ring-white/20 scale-110' : 'opacity-70 hover:opacity-100']"
                        :title="c"
                    />
                </div>
                ...
                <div class="flex items-center gap-1">
                    <!-- MEMO-05: Backlinks button -->
                    <button
                        v-if="!isNew && memo?.id"
                        @click="store.openBacklinks(memo!.id)"
                        class="p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors text-zinc-400 hover:text-zinc-800"
                        title="View Backlinks"
                    >
                        <i class="ri-link text-lg leading-none"></i>
                    </button>
            ...
            <div class="flex-1 overflow-y-auto px-8 pb-4 custom-scrollbar">
                ...
                <!-- MEMO-05: Linked Entities Display -->
                <div v-if="localData.linked_entities && localData.linked_entities.length > 0" class="mt-4 border-t border-gray-100 pt-3">
                    <div class="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mb-2">Linked Entities</div>
                    <div class="flex flex-wrap gap-2">
                        <div
                            v-for="(le, i) in localData.linked_entities"
                            :key="i"
                            class="flex items-center gap-1.5 bg-gray-50 rounded-lg px-2 py-1 text-xs text-gray-600 group"
                        >
                            <span class="text-[9px] uppercase tracking-wider text-gray-400 bg-gray-200 px-1 py-0.5 rounded">{{ le.target_type }}</span>
                            <span>{{ le.target_title }}</span>
                            <button @click="removeLinkedEntity(i)" class="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div class="relative flex-1 min-w-[120px]">
                        ...
                        >
                            <button
                                ...
                                class="px-3 py-2 text-left text-xs font-medium text-zinc-600 dark:text-zinc-300 hover:bg-black/5 dark:hover:bg-white/5 flex items-center justify-between"
                            >
            ...
            <div class="px-6 py-3 border-t border-black/5 dark:border-white/5 bg-gray-50/50 dark:bg-zinc-800/30 backdrop-blur-sm shrink-0 z-20">
                <!-- Top Row: Status, Priority, Dates -->
                <div class="flex items-center gap-3 flex-wrap">
                    <!-- Status -->
                    <div class="relative group h-8 flex items-center">
                        <select
                            v-model="localData.status"
                            class="appearance-none bg-transparent pl-6 pr-2 h-full text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:text-zinc-900 cursor-pointer rounded-lg hover:bg-black/5 transition-colors focus:ring-0 border-none"
                        >
                        ...
                        <i class="ri-checkbox-circle-line absolute left-1.5 text-base text-zinc-400 pointer-events-none"></i>
                    </div>
                    ...
                    <div class="relative group h-8 flex items-center">
                        ...
                        <i class="ri-flag-line absolute left-1.5 text-base text-zinc-400 pointer-events-none"></i>
                    </div>
                    ...
                    <div class="relative h-8 flex items-center">
                        <input
                            ...
                            placeholder="Channel..."
                            list="channel-list"
                            class="bg-transparent border-none text-[10px] font-bold uppercase tracking-widest text-zinc-500 h-full px-2 focus:ring-0 w-24 placeholder:text-zinc-300"
                        />
                        <datalist id="channel-list">
                            <option v-for="ch in store.channels" :key="ch" :value="ch" />
                        </datalist>
                    </div>
                    ...
                    <!-- Due Date -->
                    <div class="relative flex items-center group cursor-pointer hover:bg-black/5 rounded-lg px-2 h-8 transition-colors">
                        <i class="ri-calendar-line text-base text-zinc-400 group-hover:text-zinc-600 mr-1"></i>
                        <input type="datetime-local" v-model="dates.due" class="absolute inset-0 opacity-0 cursor-pointer" />
                        <span class="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                            {{ dates.due ? formatDisplayDate(dates.due) : 'Due' }}
                        </span>
                    </div>

                    <!-- MEMO-06: Reminder -->
                    <div class="relative flex items-center group cursor-pointer hover:bg-black/5 rounded-lg px-2 h-8 transition-colors">
                        <i class="ri-alarm-line text-base text-zinc-400 group-hover:text-zinc-600 mr-1"></i>
                        <input type="datetime-local" v-model="dates.reminder" class="absolute inset-0 opacity-0 cursor-pointer" />
                        <span class="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                            {{ dates.reminder ? formatDisplayDate(dates.reminder) : 'Remind' }}
                        </span>
                    </div>

                    <!-- MEMO-06: Scheduled -->
                    <div class="relative flex items-center group cursor-pointer hover:bg-black/5 rounded-lg px-2 h-8 transition-colors">
                        <i class="ri-time-line text-base text-zinc-400 group-hover:text-zinc-600 mr-1"></i>
                        <input type="datetime-local" v-model="dates.scheduled" class="absolute inset-0 opacity-0 cursor-pointer" />
                        <span class="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                            {{ dates.scheduled ? formatDisplayDate(dates.scheduled) : 'Schedule' }}
                        </span>
                    </div>
...
if (props.memo) {
    ...
    localChannel.value = props.memo.channel || '';
    if (props.memo.due_at) dates.value.due = props.memo.due_at.slice(0, 16);
    if (props.memo.reminder_at) dates.value.reminder = props.memo.reminder_at.slice(0, 16);
    if (props.memo.scheduled_at) dates.value.scheduled = props.memo.scheduled_at.slice(0, 16);
} else {
    ...
    // Inherit current filter channel
    if (store.filterChannel) {
        localChannel.value = store.filterChannel;
    }
}
...
function save() {
    ...
    if (dates.value.reminder) payload.reminder_at = new Date(dates.value.reminder).toISOString();
    if (dates.value.scheduled) payload.scheduled_at = new Date(dates.value.scheduled).toISOString();
    if (localChannel.value.trim()) payload.channel = localChannel.value.trim();
...
// MEMO-05: Linked entities
function removeLinkedEntity(index: number) {
    if (localData.value.linked_entities) {
        localData.value.linked_entities.splice(index, 1);
    }
}
</script>
